...
window.onload = function () {
    ...
    const mainPageCont = document.querySelector(".Main-Page-cont");
    ...
    hideAllSections(); // скрываем всё

    // Проверим, что элементы найдены
    if (!mainPageCont || !vidiMassaja) {
        console.warn("mainPageCont или vidiMassaja не найдены!");
        return;
    }

    // Показываем блоки
    mainPageCont.style.display = "flex";
    vidiMassaja.style.display = "flex";

    // Ждём, чтобы DOM успел обновиться
    setTimeout(() => {
        const offset = -200; // отступ вверх (например, под шапку)
        const top = vidiMassaja.getBoundingClientRect().top + window.pageYOffset + offset;

        console.log("Scroll to:", top); // <-- проверим координату
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const langLinks = document.querySelectorAll('.lang a'); // новые ссылки
